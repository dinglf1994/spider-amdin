...
//        var_dump($page);exit;
    ...
    {
        // AND label IN (2,3,4)
        $sql = "SELECT label, COUNT(articleid) as total, STR_TO_DATE(`article_pubtime_str`,'%Y') as y FROM cs_article_search WHERE UNIX_TIMESTAMP(STR_TO_DATE(`article_pubtime_str`,'%Y')) > UNIX_TIMESTAMP(STR_TO_DATE(NOW(),'%Y'))-157852900 GROUP BY STR_TO_DATE(`article_pubtime_str`,'%Y'), label ORDER BY y DESC";
        $articleSearch = new ArticleSearch();
